...
type IHOUR_INDEXES = 'morning' | 'afternoon' | 'night';
...
export class FilterUnitsService {
  ...
  trasnformWeekday(weekday: number) {
    switch (weekday) {
      case 0:
        return 'Dom.';
      case 6:
  ...
  filterUnits(unit: Location, open_hour: string, close_hour: string) {
    ...
    let todays_weekday = this.trasnformWeekday(new Date().getDay());
  ...
  filter(results: Location[], showClosed: boolean, hour: string) {
    ...
    if (hour) {
      const OPEN_HOUR =
        OPENING_HOURS[hour as IHOUR_INDEXES].first;
      const CLOSE_HOUR =
        OPENING_HOURS[hour as IHOUR_INDEXES].last;
